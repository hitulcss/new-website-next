...
export default function CustomVideoPlayer({ url, autoplay = false, muted = false }: { url: string; autoplay?: boolean; muted?: boolean }) {
  ...
  return (
    <div
      ...
      className={`relative bg-black overflow-hidden rounded-xl shadow-md transition-all duration-500
        ${isFullscreen ? 'fixed inset-0 z-50' : isMini ? 'fixed bottom-4 right-4 w-64 h-36 z-40' : 'w-full max-w-[668px] h-[220px] sm:h-[300px] md:h-[250px] lg:h-[185px]  aspect-video mx-auto'}`}
